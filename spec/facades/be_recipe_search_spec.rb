...
RSpec.describe BeRecipeSearch do
  describe 'initialize' do
    it 'has attributes' do
      search = BeRecipeSearch.new({ search: 'taco salad', ingredients: ['taco shells', 'tomatoes', 'onions'] })
  ...
  describe 'instance methods' do
    it 'search with name' do
      VCR.use_cassette('BeRecipeSearch/instance_methods/search_with_name', match_requests_on: [:path]) do
        expect(Recipe.all.count).to eq(0)
        expect(Ingredient.all.count).to eq(0)
        expect(RecipeIngredient.all.count).to eq(0)

        params = { search: 'tacos' }
        search = BeRecipeSearch.new(params)
        search.search
    ...
    it 'search with ingredients' do
      VCR.use_cassette('BeRecipeSearch/instance_methods/search_with_ingredients', match_requests_on: [:path]) do
        recipe_3 = Recipe.create!(name: 'Pasta', api_id: '1234567891234567892', image_url: 'yummy pasta url',
                                  cook_time: 10, public_status: true, source_name: 'Italian Chef', source_url: 'Italian Chef Web')
    ...
    it 'searches for Api only if there are < 10 local results' do
      ...
      expect(BeRecipeSearch.new(params).search.count).to eq(12)
